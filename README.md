...
LLM 深度推理 / 硬规则引擎 / RL 强化学习三路并行打分、独立输出，经融合决策后给出最终建议与凯利仓位。
...
11 个独立分析技能（K 线形态、RSI/MACD 背离、突破、支撑阻力、动量、量价、资金流、波动率、多周期共振等）各自扫描数据输出信号，连同技术指标快照一起交给 DeepSeek 做**全局推理**——不只算加权分，而是理解信号间的矛盾、印证与市场环境关联。
